        ...
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              data-name={name}
              className={cn(
                "group relative flex items-center gap-3 rounded-xl border bg-white px-4 text-left transition-all duration-200",
                size === "md" ? "py-3" : "py-4",
                selected
                  ? "border-primary bg-bg-base shadow-glow"
                  : "border-border hover:border-primary hover:bg-bg-base/50"
              )}
            >
              {opt.emoji && (
                <span
